...
| Shared gallery photography composer | `ComfyStewardView` | `lab/`, `tools/era-archive/capture_catalog.py` | versioned composer assets, camera fixtures and capture catalog | asset hashes, camera tests and measured download gate |
| Portable still capture runner and game camera | [`SelfieStick`](https://github.com/djcdevelopment/SelfieStick) | `runner/`, `Plugin.cs`, `ExactCapture.cs` | `selfiestick-capture/v1`, pinned runner ZIP and DLL manifests | archive identity, measured camera/PNG receipts and save restoration |
| World snapshot analytics, multi-era gallery and spatial map authoring | [`ComfyStewardView`](https://github.com/djcdevelopment/ComfyStewardView) | `viewer/`, `lab/`, `tools/era-archive/`, migrated `tools/selfie-stick/` helpers | verified Parquet/DuckDB packages, creator projections and world catalogs | source hashes, exact membership, public projection and Steward contract tests |
