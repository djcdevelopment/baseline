...
- Source: Etheiry (Luna)'s "Creator Events Tracker, Guidelines & Schedule" drop
  (archived in the comfy archive repo, comfy-etheiry-analysis/CreatorStuff/).
...
- Source: Luna's "Hobbit Reqs & Rewards + Reset Checklist" copy, shared on Discord
  2026-08-07 (archived in the comfy archive repo).
...
- Harvested: `E17 Reqs & Rewards` via the `hobbit-ladder-xlsx` adapter (kind:
  ...
  "Proposed E17 Rewards" column, current-era diffs surfaced as anomalies.
- Deliberately not harvested: the reset checklists (ops runbook) and `Completion
  Data` (demand data for a future telemetry cross-check).
