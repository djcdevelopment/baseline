...
| Lab runtime provenance | `fieldlab/scripts/Test-LabRuntimeProvenance.ps1 -AllowRetainedStateBridge -RetainedStateRoot C:\work\comfy\fieldlab\autonomous\state` passed against `comfy-valheim-lab-valheim-server-1`: Baseline Compose file/working directory, no retired Compose source, server image digest `sha256:e8b13da3c44f54a38511c8ac224f2959a437c0b2626cf916683ca7acc8dfb146`, explicit credential-free OMEN/i5 routes, and `state_root_disposition=retained_legacy_bridge`. The rendered runner invokes the strict form first and never admits the interim bridge. Job `job-20260802-054530209-f67cfc0d` therefore failed with `rendered_prelive_lab_provenance_failed` and `state_root_disposition=unknown` before source/image checks, i5 preflight, scenario generation, or client launch; it created no run artifacts, both clients remained stopped, i5's task remained `Ready`, and OMEN retained safe config hash `A141231B84010B456537B2B80360C3C0128FED2753ACFAD76DC24310A71F1F42`. | Strict rendered gate passed; migration pending |
