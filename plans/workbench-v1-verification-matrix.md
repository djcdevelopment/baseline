...
| Operate slice | Existing v0 check/install/rollback/capture adapters are wired. The original `operate.mod.check` produced deterministic `gateway_manifest_unavailable`; clean Lab job `job-20260802-013242046-49017a94` passed with `mod_manifest_read` against local candidate `m32-workbench-20260802-r1`. Its byte preflight matched all 30 live OMEN payload entries with zero different, missing, or unsafe entries. Topology truthfully showed Local Gateway ready and P7 excluded | Read-only check and no-difference preflight passed; install/rollback and player-active transport capture remain operator-gated because they affect or depend on player-facing state |
...
| Dev MCP endpoint provenance | Current candidate HEAD `625c033f4b42af51e67557257e855700f0a0e0b9` returned `baseline.mcp.identity.v1` with `source_dirty=false` and matched project, Lab profile, image tag, published port `8721`, Workbench provider, caller registry, and Baseline ledger; immutable Dev MCP image ID `sha256:818848118f1888ea1df82f1a047b36fc6381576deebd23a5588701374284f5f6` is recorded separately | Passed |
