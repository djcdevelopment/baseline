...
| Origin | Status | Notes |
|---|---|---|
| `https://am4.tail8e749c.ts.net` | **LIVE** | The community-facing origin, exposed by Tailscale Funnel. Serves `/workbench`, `/community`, `/roadmap`, `/questlab`, `/networksense`, `/events`, `/testing`, `/health`, `/workbench/downloads/*`. **This is the only public origin.** ⚠️ `/questpicker` and `/steward` appear in every page's nav bar but **no route for either exists in this repo's Gateway source** — whatever serves them lives on the AM4 host outside this repo, and nothing here can verify or redeploy them. |
