...
flowchart TD
    ...
    Setup --> Panel["F6 opens the saved, zoomable panel: events / spellbook / scenarios / quests"]
...
### C. Safe creator draft export — **BUILT; completion submission remains deferred**
...
The non-conflicting half now exists. The Scenarios tab can copy an ordinary editable schema-1
quest generated by `QuestAuthoring`, or prepare it beside a deterministic manifest under one
fixed `comfy-quest-lab/scenarios/scenario-<event>/` folder. It never writes to an outbox, arms the
draft, or edits creator quest files. Re-preparing reuses identical output and refuses to
overwrite a changed file. Completion submission remains deliberately absent.
...
events. Its receipt says `synthetic-contract`; it cannot graduate a school to “witnessed”. The
34 one-event `scenario-<event>` rehearsals make that contract discoverable: prepare exports the
editable draft and manifest, run asks the exact shared evaluator, and the receipt says
`synthetic-scenario`. The i5 helper can deliver only eleven expiring, allowlisted suite/gallery
operations through the SHA-verified config lane. The two release suites and all 34 scenario IDs
are explicit allowlists. It has no general console, keystroke, arbitrary path, or prefab field.
...
4. **Experimentation** — browse Scenarios, copy or prepare one exact draft, edit JSON,
   `lab_reload`, and read the Quests tab.
