...
the fonts and the five diagrams into `template.html`, and writes `site/index.html`,
`site/og.jpg` and `site/.nojekyll`.
...
`site/index.html` is a build output, but it is committed — it is what Pages serves and
it is the only surviving copy of the curated images once `build/` is cleaned. Re-run
both scripts and commit the result; do not hand-edit it.
